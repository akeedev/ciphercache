...
## What it is
`ciphercache` is a local secret agent that unlocks a KeePassXC database on demand,
caches requested secrets in memory for a TTL, and serves secrets over local IPC.
...
- Python client SDK.
- Explicit unlock with secret list; no implicit unlock on request.
